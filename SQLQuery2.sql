...
AS
BEGIN
INSERT INTO LOS_GESTIONADORES.anuncio(anuncio_id, fecha_publicacion, precio_publicado, costo_anuncio, fecha_finalizacion, agente_id, moneda_id, tipo_operacion_id, inmueble_id,
...
INNER JOIN LOS_GESTIONADORES.agente A ON (M.AGENTE_DNI = A.dni)
...
INNER JOIN LOS_GESTIONADORES.inmueble I ON (M.INMUEBLE_NOMBRE = I.nombre_inmueble  AND M.INMUEBLE_CODIGO = I.inmueble_id)
...
AS
BEGIN
	INSERT INTO LOS_GESTIONADORES.Inmuebles(inmueble_id, tipo_inmueble_id, descripcion, propietario_id, direccion_id, tipo_ambiente_id, superficie_total, disposicion_id, orientacion_id, estado_id, antiguedad, expensas, nombre_inmueble)
	SELECT DISTINCT 
		M.INMUEBLE_CODIGO,
		T.tipo_inmueble_id,
...
A.tipo_ambiente_id, 
	...
	INNER JOIN LOS_GESTIONADORES.Direccion D ON M.INMUEBLE_DIRECCION = (D.calle+'' +D.altura)
	INNER JOIN LOS_GESTIONADORES.TipoAmbiente A ON M.INMUEBLE_CANT_AMBIENTES = A.descripcion
...
INNER JOIN LOS_GESTIONADORES.INQUILINO I ON M.INQUILINO_DNI = I.DNI
...
AS
BEGIN
	INSERT INTO LOS_GESTIONADORES.COMPRADOR(nombre, apellido, mail, telefono, dni, fecha_nacimiento, fecha_registro)
SELECT DISTINCT 
COMPRADOR_NOMBRE,
COMPRADOR_APELLIDO,
COMPRADOR_MAIL,
...
BEGIN
	...
	(SELECT provincia_id FROM LOS_GESTIONADORES.PROVINCIA WHERE nombre = gd_esquema.Maestra.SUCURSAL_PROVINCIA)
    FROM gd_esquema.Maestra 
    WHERE SUCURSAL_LOCALIDAD NOT IN (SELECT nombre FROM LOS_GESTIONADORES.LOCALIDAD);

	INSERT INTO LOS_GESTIONADORES.LOCALIDAD (nombre, provincia_id)
    SELECT DISTINCT INMUEBLE_LOCALIDAD,
	(SELECT provincia_id FROM LOS_GESTIONADORES.PROVINCIA WHERE nombre = gd_esquema.Maestra.INMUEBLE_LOCALIDAD)
    FROM gd_esquema.Maestra 
    WHERE INMUEBLE_LOCALIDAD NOT IN (SELECT nombre FROM LOS_GESTIONADORES.LOCALIDAD);

    --  BARRIO
    INSERT INTO LOS_GESTIONADORES.BARRIO (nombre, localidad_id)
    SELECT DISTINCT INMUEBLE_BARRIO,
	(SELECT localidad_id FROM LOS_GESTIONADORES.LOCALIDAD WHERE nombre = gd_esquema.Maestra.INMUEBLE_LOCALIDAD)
    FROM gd_esquema.Maestra
    WHERE INMUEBLE_BARRIO NOT IN (SELECT nombre FROM LOS_GESTIONADORES.BARRIO)

    -- DIRECCION
    INSERT INTO LOS_GESTIONADORES.DIRECCION (calle, altura, barrio_id)
    SELECT
        LEFT(SUCURSAL_DIRECCION, PATINDEX('%[0-9]%', SUCURSAL_DIRECCION) - 1) AS calle,
        SUBSTRING(SUCURSAL_DIRECCION, PATINDEX('%[0-9]%', SUCURSAL_DIRECCION), LEN(SUCURSAL_DIRECCION)) AS altura,
        (SELECT barrio_id FROM LOS_GESTIONADORES.BARRIO WHERE nombre = gd_esquema.Maestra.INMUEBLE_BARRIO)
    FROM gd_esquema.Maestra
...
AS
BEGIN
    INSERT INTO LOS_GESTIONADORES.SUCURSAL (sucursal_id, direccion_id, telefono)
    SELECT DISTINCT
        SUCURSAL_CODIGO AS sucursal_id,
        (SELECT direccion_id FROM LOS_GESTIONADORES.DIRECCION WHERE calle = LEFT(SUCURSAL_DIRECCION, PATINDEX('%[0-9]%', SUCURSAL_DIRECCION) - 1) AND altura = SUBSTRING(SUCURSAL_DIRECCION, PATINDEX('%[0-9]%', SUCURSAL_DIRECCION), LEN(SUCURSAL_DIRECCION))) AS direccion_id,
...
BEGIN
    ...
    SELECT DISTINCT 
        ...
        MP.medio_pago_id 
    FROM gd_esquema.Maestra M
    LEFT JOIN LOS_GESTIONADORES.VENTAS V ON M.PAGO_VENTA_COTIZACION = V.cotizacion
    LEFT JOIN LOS_GESTIONADORES.TIPO_MONEDA TM ON M.PAGO_VENTA_MONEDA = TM.nombre
...
INSERT INTO LOS_GESTIONADORES.Caracteristica_Inmueble (inmueble_id, caracteristica_id)
SELECT INMUEBLE_CODIGO, c.caracteristica_id
FROM gd_esquema.Maestra m
...
inmueble_id numeric(18,0) PRIMARY KEY,
...
inmueble_id numeric(18,0) FOREIGN KEY REFERENCES LOS_GESTIONADORES.inmueble(inmueble_id),
...
nro_periodo_inicio numeric(18,0) NOT NULL,
nro_periodo_fin numeric(18,0)NOT NULL,
