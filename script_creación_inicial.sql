...
BEGIN
    ...
    ELSE SUBSTRING(M.INMUEBLE_DIRECCION, PATINDEX('%[0-9]%', M.INMUEBLE_DIRECCION), LEN(M.INMUEBLE_DIRECCION)) 
	END,
    ...
    ELSE SUBSTRING(M.SUCURSAL_DIRECCION, PATINDEX('%[0-9]%', M.SUCURSAL_DIRECCION), LEN(M.SUCURSAL_DIRECCION)) 
	END
    FROM gd_esquema.Maestra M
...
AS
BEGIN
    INSERT INTO LOS_GESTIONADORES.SUCURSAL (sucursal_id, telefono, direccion_id)
    SELECT 
